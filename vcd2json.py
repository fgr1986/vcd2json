...
class WaveExtractor:
    ...
    def __init__(self, vcd_file, json_file, path_list, states_dict: dict = {}):
        """
    ...
    def _setup(self):
        ...
        def create_path_dict(fin):
            ...
            while True:
                ...
                if words[0] == "$scope":
                    ...
                    path_dict[path] = _SignalDef(
                        name=words[4], sid=words[3], length=int(words[2])
                    )
        ...
        def update_path_dict(path_list, path_dict):
            ...
            for path in path_list:
                ...
                if not signal_def:
                    raise ValueError(f'Can\'t find path "{path}".')
        ...
        for p in path_dict.items():
            print("DEBUG ", p[0], " sid: ", p[1]._sid)
        if self._path_list:
    ...
    def execute(self):
        ...
        states_dict = {
            path_dict[k]._sid: d_val for k, d_val in self._states_dict.items()
        }
        ...
        while True:
            ...
            if len(sample_dict[clock_id]) == 0:
                break
            fout.write(jsongen.create_body(origin, sample_dict, states_dict))
...
class _JsonGenerator:
    ...
    def __init__(self, path_list, path_dict, wave_chunk):
        ...
        self._name_width = max([len(path_dict[path]._name) for path in path_list])
    ...
    def create_body(self, origin, sample_dict, states_dict: dict):
        ...
        def create_wave_data(samples, length, fmt, sid, states_dict):
            prev = None
            wave = ""
            data = ""
            for value in samples:
                if value == prev:
                    ...
                    wave += "="
                    data += " " + data_format(
                        value, length, fmt, sid=sid, states_dict=states_dict
                    )
        ...
        def data_format(value, length, fmt, sid, states_dict):
            value = int(value, 2)
            if fmt == "b":
                fmt = "0" + str(length) + "b"
            elif fmt == "d":
                ...
                fmt = "0" + str((length + 3) // 4) + "x"
            if sid in states_dict:
                enc = states_dict[sid][value]
                print(enc)
                return enc
        ...
        for path in self._path_list[1:]:
            ...
            if length == 1:
                ...
                name = '"{0}"'.format(name).ljust(self._name_width + 2)
                wave, data = create_wave_data(
                    sample_dict[sid], length, fmt, sid=sid, states_dict=states_dict
                )
                json += (
                    ',\n    { "name": '
                    + name
